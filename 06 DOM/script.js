...
data.forEach(({id, title, price, image}) =>{
    ...
    <div class="imagen"><img src="${image}"/></div>
    <div class="precio">${price}</div>
    <div class="botones">
        <button class="boton" id="menos-${id}">-</button>
        <span class="cantidad" id="cantidad-${id}">0</span>
        <button class="boton" id="mas-${id}">+</button>
    </div>
    `;
    productos.append(div);

    const menos = document.getElementById(`menos-${id}`);
    const mas = document.getElementById(`mas-${id}`);
